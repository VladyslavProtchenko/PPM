...
interface some { [key: number]: boolean}
        ...
        <div className={container}>
            {
                content.map((item, index)=>(
                        ...
                        {!isEdit[index]?<FiEdit className={editIcon} onClick={()=>{
                        ...
                        <div className={editField}>
                            <Input placeholder='title' onChange={(e)=>{
                                setData(data.map(item => item.section === index+1 ? {...item, title:e.target.value }: item ))
                            }}/>
                            <Input placeholder='title 2' onChange={(e)=>{
                                setData(data.map(item => item.section === index+1 ? {...item, title2:e.target.value }: item ))
                            }}/>
                            <Input placeholder='text' onChange={(e)=>{
                                setData(data.map(item => item.section === index+1 ? {...item, text:e.target.value }: item ))
                            }}/>
                            <Input placeholder='text 2' onChange={(e)=>{
                                setData(data.map(item => item.section === index+1 ? {...item, text2:e.target.value }: item ))
                            }}/>
                        </div>}
